...
public class AdminTReviewServiceImpl implements AdminTReviewService {
	...
	public void insertTReview(TReviewReqDTO treviewReqDTO, MultipartFile[] imageFiles) {
		try {
			...
			adminTReviewDAO.insertTReview(tReviewVO);
			...
			if (imageFiles != null && imageFiles.length > 0) {
				for (MultipartFile imageFile : imageFiles) {
					if (!imageFile.isEmpty()) {
						...
						adminTReviewDAO.insertTReviewImage(tReviewImageVO);
					}
				}
			}
	...
	public void incrementTReviewCount(int trevId) {
		try {
			adminTReviewDAO.incrementTReviewCount(trevId);
		} catch (DataAccessException e) {
	...
	public void editTReview(TReviewReqDTO treviewReqDTO, MultipartFile[] imageFiles, List<String> existingImages) {
		try {
			...
			treviewReqDTO.validate();
			TReviewResDTO treviewResDTO = adminTReviewDAO.getTReviewById(treviewReqDTO.getTrevId());
			TReviewVO tReviewVO = TReviewVO.fromDTO(treviewReqDTO, treviewResDTO);

			adminTReviewDAO.editTReview(tReviewVO);

			// 기존 이미지 삭제
			adminTReviewDAO.deleteTReviewImages(tReviewVO.getTrevId());
	...
	public void deleteTReview(int trevId) {
		try {
			...
			TReviewResDTO treviewResDTO = adminTReviewDAO.getTReviewById(trevId);
			String trevContent = treviewResDTO != null ? treviewResDTO.getTrevContent() : null;

			adminTReviewDAO.deleteTReview(trevId);
	...
	public void reportTReview(int trevId, String reportedContentType, String reportReason) {
		try {
			...
			adminTReviewDAO.reportTReview(trevId);
			adminTReviewDAO.insertReportTReview(reportVO);
